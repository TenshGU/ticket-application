...
import "../../static/style/pages/BuyTicket.css";
  ...
  componentDidMount = () => {
              ...
              data.push({
                ...
                leave: (
                  <div className="listTime">
                    ...
                    <div className="listTimep2">{item.leaveAirportName}</div>
                  </div>
                ),
                arrive: (
                  <div className="listTime">
                    <div className="listTimep1">{item.arriveTime}</div>
                    <div className="listTimep2">{item.arriveAirportName}</div>
                  </div>
                ),
                price: (
                  <Space size="middle">
    ...
    axios
      ...
      .then((res) => {
        if (res.data.code == 200) {
          this.setState({
            airports: res.data.infoMap.airports,
          });
          console.log(res.data.infoMap.airports);
        } else {
          this.props.history.push("login");
          message.warning("请重新登录。");
        }
              ...
              <Content style={{ padding: "2% 13%" }}>
                <div id="card1" className="card11">
                  <Form
                    layout="horizontal"
                    size="large"
                    onFinish={this.updatelist}
                  >
                    <Row style={{ height: "50px", marginTop: "30px" }}>
                      <Col
                        xs={{ span: 24 }}
                        sm={{ span: 24 }}
                        md={{ span: 24 }}
                        ...
                        xl={{ offset: 3, span: 5 }}
                      >
                        <Form.Item name="leave" label="出发地：">
                          <Select onChange={this.onGenderChange1}>
                            {!this.state.airports ? (
                      ...
                      <Col
                        ...
                        xl={{ offset: 1, span: 5 }}
                      >
                        <Form.Item name="arrive" label="目的地：">
                          <Select onChange={this.onGenderChange2}>
                            {!this.state.airports ? (
                              <Select.Option value="demo">
                                暂无数据
                              </Select.Option>
                      ...
                      <Col
                        ...
                        xl={{ offset: 1, span: 8 }}
                      >
                        <Form.Item name="leaveTime" label="选择日期：">
                          <DatePicker onChange={this.onGenderChange1} />
                        </Form.Item>
                      </Col>
                    </Row>
                    <Row>
